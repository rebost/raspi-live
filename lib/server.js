...
module.exports = (directory, format, width, height, framerate, horizontalFlip, verticalFlip, rotation, compressionLevel, time, listSize, storageSize, port, nopreview) => {
  ...
  const raspividOptions = ['-o', '-', '-t', '0', '-w', width, '-h', height, '-fps', framerate, '-g', framerate, '-rot', rotation];
  if (horizontalFlip) raspividOptions.push('-hf');
  if (verticalFlip) raspividOptions.push('-vf');
  if (nopreview) raspividOptions.push('-n');
